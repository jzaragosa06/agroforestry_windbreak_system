...
function updateWindLayer() {
  ...
  if (polygon) {
    ...
    var meanWindDirectionRad = meanWindDirection.multiply(Math.PI / 180); 

    // Calculate alignment of aspect with  wind direction
    // cos(aspect - WindDirection) gives high values when they align
    var alignment = aspectRad.subtract(meanWindDirectionRad).cos().abs();
    ...
    Map.addLayer(highlighted, highlightVis, 'Elevation Against Wind (EAW)', true, 0.7);
    Map.addLayer(highlighted_mask, highlighted_mask_vis, "EAW - Highlighted Mask (Rate: " + maskRate + ")", true, 0.7);
    ...
    meanWindDirection.evaluate(function (windDir) {
      ...
      // Add explanation
      resultsPanel.add(ui.Label({
        value: 'White areas show elevation features against the prevailing wind direction.',
        style: { margin: '10px 0 5px 0', fontSize: '12px' }
      }));
    });

    // Resistance features scale
    var resistanceLabel = ui.Label('Resistance Features');
    var resistanceGradient = ui.Thumbnail({
    ...
    legend.add(resistanceLabel);
    legend.add(resistanceGradient);
...
var controlPanel = ui.Panel({
  widgets: [
    ui.Label({
      value: 'Wind Against Elevation Analysis',
      style: { fontWeight: 'bold', fontSize: '16px', margin: '0 0 10px 0' }
    }),
...
// Instructions
print('Draw a polygon on the map to analyze elevation features that has high resistance to mean wind direction.');
